 ...
 * 
 */
public class GroupHeader46 extends ComplexDataType {
	...
	/**
	 * Constructor for GroupHeader46.
	 *
	 * @param elementName the name of the originating XML tag
	 * @param parent the parent data
	 */
	public GroupHeader46(String elementName, ComplexDataType parent) {
	 ...
	 * The allocator is used by the framework to instantiate type GroupHeader46.
	 */
	static class Allocator implements TypeAllocator<GroupHeader46> {
		/**
		 * method for getting a new instance of type GroupHeader46.
		 *
		 * @param elementName the name of the originating XML tag
		 * @param parent the parent data
		 * @return new instance
		 */
		public GroupHeader46 newInstance(String elementName, ComplexDataType parent) {
			return new GroupHeader46(elementName, parent);
		}
	}
	...
	public boolean equals(Object that) {  
		...
		if (!Compare.equals(m_msgId, ((GroupHeader46)that).m_msgId))
			return false;
		
		if (!Compare.equals(m_creDtTm, ((GroupHeader46)that).m_creDtTm))
			return false;
		
		if (!Compare.equals(m_initgPty, ((GroupHeader46)that).m_initgPty))
			return false;
		
		if (!Compare.equals(m_dbtrAgt, ((GroupHeader46)that).m_dbtrAgt))
			return false;
		
		if (!Compare.equals(m_cdtrAgt, ((GroupHeader46)that).m_cdtrAgt))
			return false;
