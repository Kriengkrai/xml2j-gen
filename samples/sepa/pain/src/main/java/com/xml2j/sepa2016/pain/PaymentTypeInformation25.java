...
/**
 * PaymentTypeInformation25 data class.
 *
 ...
 * 
 */
public class PaymentTypeInformation25 extends ComplexDataType {
	...
	/**
	 * Constructor for PaymentTypeInformation25.
	 *
	 * @param elementName the name of the originating XML tag
	 * @param parent the parent data
	 */
	public PaymentTypeInformation25(String elementName, ComplexDataType parent) {
	 ...
	 * The allocator is used by the framework to instantiate type PaymentTypeInformation25.
	 */
	static class Allocator implements TypeAllocator<PaymentTypeInformation25> {
		/**
		 * method for getting a new instance of type PaymentTypeInformation25.
		 *
		 * @param elementName the name of the originating XML tag
		 * @param parent the parent data
		 * @return new instance
		 */
		public PaymentTypeInformation25 newInstance(String elementName, ComplexDataType parent) {
			return new PaymentTypeInformation25(elementName, parent);
		}
	}
	...
	public boolean equals(Object that) {  
		...
		if (!Compare.equals(m_instrPrty, ((PaymentTypeInformation25)that).m_instrPrty))
			return false;
		
		if (!Compare.equals(m_clrChanl, ((PaymentTypeInformation25)that).m_clrChanl))
			return false;
		
		if (!Compare.equals(m_svcLvl, ((PaymentTypeInformation25)that).m_svcLvl))
			return false;
		
		if (!Compare.equals(m_lclInstrm, ((PaymentTypeInformation25)that).m_lclInstrm))
			return false;
		
		if (!Compare.equals(m_seqTp, ((PaymentTypeInformation25)that).m_seqTp))
			return false;
		
		if (!Compare.equals(m_ctgyPurp, ((PaymentTypeInformation25)that).m_ctgyPurp))
			return false;
