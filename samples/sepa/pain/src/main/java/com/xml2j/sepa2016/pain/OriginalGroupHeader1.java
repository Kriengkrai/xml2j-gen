...
/**
 * OriginalGroupHeader1 data class.
 *
 ...
 * 
 */
public class OriginalGroupHeader1 extends ComplexDataType {
	...
	/**
	 * Constructor for OriginalGroupHeader1.
	 *
	 * @param elementName the name of the originating XML tag
	 * @param parent the parent data
	 */
	public OriginalGroupHeader1(String elementName, ComplexDataType parent) {
	 ...
	 * The allocator is used by the framework to instantiate type OriginalGroupHeader1.
	 */
	static class Allocator implements TypeAllocator<OriginalGroupHeader1> {
		/**
		 * method for getting a new instance of type OriginalGroupHeader1.
		 *
		 * @param elementName the name of the originating XML tag
		 * @param parent the parent data
		 * @return new instance
		 */
		public OriginalGroupHeader1 newInstance(String elementName, ComplexDataType parent) {
			return new OriginalGroupHeader1(elementName, parent);
		}
	}
	...
	public boolean equals(Object that) {  
		...
		if (!Compare.equals(m_orgnlMsgId, ((OriginalGroupHeader1)that).m_orgnlMsgId))
			return false;
		
		if (!Compare.equals(m_orgnlMsgNmId, ((OriginalGroupHeader1)that).m_orgnlMsgNmId))
			return false;
		
		if (!Compare.equals(m_orgnlCreDtTm, ((OriginalGroupHeader1)that).m_orgnlCreDtTm))
			return false;
		
		if (!Compare.equals(m_orgnlNbOfTxs, ((OriginalGroupHeader1)that).m_orgnlNbOfTxs))
			return false;
		
		if (!Compare.equals(m_orgnlCtrlSum, ((OriginalGroupHeader1)that).m_orgnlCtrlSum))
			return false;
		
		if (!Compare.equals(m_grpSts, ((OriginalGroupHeader1)that).m_grpSts))
			return false;
		
		if (!Compare.equals(m_stsRsnInfList, ((OriginalGroupHeader1)that).m_stsRsnInfList))
			return false;
		
		if (!Compare.equals(m_nbOfTxsPerStsList, ((OriginalGroupHeader1)that).m_nbOfTxsPerStsList))
			return false;
