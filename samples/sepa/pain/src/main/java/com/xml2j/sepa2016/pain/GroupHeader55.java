 ...
 * 
 */
public class GroupHeader55 extends ComplexDataType {
	...
	/**
	 * Constructor for GroupHeader55.
	 *
	 * @param elementName the name of the originating XML tag
	 * @param parent the parent data
	 */
	public GroupHeader55(String elementName, ComplexDataType parent) {
	 ...
	 * The allocator is used by the framework to instantiate type GroupHeader55.
	 */
	static class Allocator implements TypeAllocator<GroupHeader55> {
		/**
		 * method for getting a new instance of type GroupHeader55.
		 *
		 * @param elementName the name of the originating XML tag
		 * @param parent the parent data
		 * @return new instance
		 */
		public GroupHeader55 newInstance(String elementName, ComplexDataType parent) {
			return new GroupHeader55(elementName, parent);
		}
	}
	...
	public boolean equals(Object that) {  
		...
		if (!Compare.equals(m_msgId, ((GroupHeader55)that).m_msgId))
			return false;
		
		if (!Compare.equals(m_creDtTm, ((GroupHeader55)that).m_creDtTm))
			return false;
		
		if (!Compare.equals(m_authstnList, ((GroupHeader55)that).m_authstnList))
			return false;
		
		if (!Compare.equals(m_nbOfTxs, ((GroupHeader55)that).m_nbOfTxs))
			return false;
		
		if (!Compare.equals(m_ctrlSum, ((GroupHeader55)that).m_ctrlSum))
			return false;
		
		if (!Compare.equals(m_initgPty, ((GroupHeader55)that).m_initgPty))
			return false;
		
		if (!Compare.equals(m_fwdgAgt, ((GroupHeader55)that).m_fwdgAgt))
			return false;
